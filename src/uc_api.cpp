...
static duckdb_yyjson::yyjson_val *GetTableMetadata(const string &catalog, const string &schema, const string &table, UCCredentials credentials) {
	...
	auto api_result = GetRequest(
		credentials.endpoint + "/api/catalog/v1/" + catalog + "/namespaces/" + schema + "/tables/" + table, 
		credentials.token,
		extra_headers);
...
static UCAPIColumnDefinition ParseColumnDefinition(duckdb_yyjson::yyjson_val *column_def) {
	...
	result.precision = -1; //TryGetNumFromObject(column_def, "type_precision");
	result.scale = -1; //TryGetNumFromObject(column_def, "type_scale");
	result.position = TryGetNumFromObject(column_def, "id") - 1;

	return result;
}

UCAPITableCredentials UCAPI::GetTableCredentials(const string &catalog, const string &schema, const string &table, UCCredentials credentials) {
	UCAPITableCredentials result;

	duckdb_yyjson::yyjson_val *root = GetTableMetadata(catalog, schema, table, credentials);
...
string UCAPI::GetToken(string id, string secret, string endpoint) {
	// curl -X POST "http://localhost:8181/api/catalog/v1/oauth/tokens" \
                                                       -H "Content-Type: application/x-www-form-urlencoded" \
                                                       -d "grant_type=client_credentials&client_id=0ce689e7bdbd6b90&client_secret=2bc0c62fd049a619e7a08024ee8d9aab&scope=PRINCIPAL_ROLE:ALL"
	string post_data = "grant_type=client_credentials&client_id=" + id + "&client_secret=" + secret + "&scope=PRINCIPAL_ROLE:ALL";
	string api_result = PostRequest(endpoint + "/api/catalog/v1/oauth/tokens", post_data);
...
vector<UCAPITable> UCAPI::GetTables(const string &catalog, const string &schema, UCCredentials credentials) {
	...
	auto api_result = GetRequest(credentials.endpoint + "/api/catalog/v1/" + catalog + "/namespaces/" + schema + "/tables", credentials.token);
	...
	yyjson_arr_foreach(tables, idx, max, table) {
		...
		auto *metadata_root = GetTableMetadata(catalog, schema, table_result.name, credentials);
...
vector<UCAPISchema> UCAPI::GetSchemas(const string &catalog, UCCredentials credentials) {
	...
	auto api_result =
	    GetRequest(credentials.endpoint + "/api/catalog/v1/" + catalog + "/namespaces", credentials.token);
